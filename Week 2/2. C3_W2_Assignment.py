...
def data_generator(batch_size, max_length, data_lines, line_to_tensor=line_to_tensor, shuffle=True):
    ...
    while True:
        ...
        if index < len(lines_index) and lines_index[index] is not None:
...
def train_model(model, data_generator, lines, eval_lines, batch_size=32, max_length=64, n_steps=1, output_dir='model/'): 
    ...
    train_task = training.TrainTask( 
        ...
        optimizer=optimizers.Adam(learning_rate=0.0005)     # Don't forget to add the learning rate parameter TO 0.0005
    ) 
